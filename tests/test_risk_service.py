...
from app.schemas.risk import RiskCreate, RiskUpdate as RiskUpdateSchema
...
class TestRiskService:
    ...
    def test_create_risk(self, mock_datetime, db_session):
        ...
        update_log = db_session.query(RiskUpdate).filter(
            RiskUpdate.risk_id == risk.risk_id
        ).first()
    ...
    def test_update_risk_exists(self, db_session, sample_risks):
        ...
        if risk.current_risk_rating != 12:  # Previous rating from sample data
            update_log = db_session.query(RiskUpdate).filter(
                RiskUpdate.risk_id == "TR-2024-CYB-001"
            ).first()
            if update_log:
    ...
    def test_create_update_log(self, db_session):
        ...
        update_log = db_session.query(RiskUpdate).filter(
            RiskUpdate.risk_id == "TR-2024-TEST-001"
        ).first()
    ...
    def test_get_recent_risk_updates(self, db_session):
        ...
        updates_data = [
            ("UPD-TR-2024-TEST-001-01", "TR-2024-TEST-001", date.today() - timedelta(days=3), "Update 1"),
            ("UPD-TR-2024-TEST-001-02", "TR-2024-TEST-001", date.today() - timedelta(days=2), "Update 2"),
            ("UPD-TR-2024-TEST-001-03", "TR-2024-TEST-001", date.today() - timedelta(days=1), "Update 3"),
        ]
